...
lesfichiersPDF=listdir("etudedossier2022/fiches")
associationColonnes={
...
def prepare_selection(context,request):
    def recupere_selection(utilise_sauvegarde=True):
        ...
        if utilise_sauvegarde:
            try:
                obj=SauvegardeSelection.objects.get(user=request.user)
                if obj.pasexpert:
        ...
        if "choixAction" in request.POST:
            if request.POST["choixAction"]=="sauvegarde":
                try:
                    ...
                    obj=SauvegardeSelection(user=request.user)
...
def convertion_excel():
    global engine
    try:
        remove('etudedossier2022/stockage/versionxls.db')
    except:
        pass
    df=pd.read_excel('etudedossier2022/stockage/fichierinitial.xlsx')
    ...
    for prof in gretudedossier.user_set.all():
        obj,created=SauvegardeSelection.objects.update_or_create(user=prof)
        obj.valeur=dumps([])
        obj.ordre=dumps([])
        #obj.boursier=False
        #obj.pasexpert=False
...
def convertion_xslx():
    ...
    df.to_excel('etudedossier2022/stockage/fichierfinal.xlsx',sheet_name='parcoursup')    
...
def extraction_donnees(request):
    ...
    try:
        ...
        if request.user.username in ["nizon","bouissou"]:
            ...
            classe=""        
        try:
            obj=AnciensEleves.objects.get(annee=2022,num_dossier=num_dossier)
            obj.commentaire=commentaire
            obj.save()
        except:
            AnciensEleves(annee=2022,classe=classe,num_dossier=num_dossier,rne=rne,prenom=prenom,nom=nom,note_initiale=note_initiale,note_finale=note_finale,
