...
from .forms import GestionDossier2021Form
...
def selection(request,err=False):
    ...
    if err:
        context['erreur']="oui"
    return render(request,'etudedossier2021/selection.html',context)

@auth([groupe_etudedossier])
def affichage(request):
    context={}
    lire_tous_les_dossiers(request,context)
    return render(request,'etudedossier2021/affichage.html',context)
...
def gestion(request):
    context={}
    form=GestionDossier2021Form()
    if request.method=='POST' and (request.user.username in ["nizon","rezzouk"]):
        if request.POST["valeur"]=='upload_initial':
            old=GestionDossier2021.objects.all()
            for x in old:
                x.fichier_initial.delete()
                x.delete()
            try:
                remove('etudedossier2021/stockage/fichierinitial.xlsx')
            except:
                pass
            form=GestionDossier2021Form(request.POST,request.FILES) 
            if form.is_valid():
                form.save()
    ...
    context["form"]=form
    return render(request,'etudedossier2021/gestion.html',context)

@auth([groupe_etudedossier])
def download(request,nom):
    try:
        if nom=='fichierfinal.xlsx':
            document=open('etudedossier2021/stockage/fichierfinal.xlsx','rb')
        else:
            nom=nom[2:-1].encode()
            nom=(base64.b64decode(nom)).decode("utf-8")
            document=open('etudedossier2021/fiches/'+nom,'rb')
