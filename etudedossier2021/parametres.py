...
lesfichiersPDF=listdir("etudedossier2021/fiches")
associationColonnes={
...
def prepare_selection(context,request):
    def recupere_selection(utilise_sauvegarde=True):
        ...
        if utilise_sauvegarde:
            try:
                obj=SauvegardeSelection2021.objects.get(user=request.user)
                if obj.pasexpert:
        ...
        if "choixAction" in request.POST:
            if request.POST["choixAction"]=="sauvegarde":
                try:
                    ...
                    obj=SauvegardeSelection2021(user=request.user)
...
def liste_anciens(n):
    liste=AnciensEleves.objects.filter(annee=2021,classe="MPSI"+str(n)).order_by("nom")
    l=[]
    for line in liste:
    ...
    with engine.connect() as conn :
        ...
        for x in associationColonnes:
            try:
                ...
                pass
        dossier["commentaire_ancien"]=trouve_com_ancien(2021,dossier["numeroDossier"])
...
def convertion_excel():
    global engine
    try:
        remove('etudedossier2021/stockage/versionxls.db')
    except:
        pass
    df=pd.read_excel('etudedossier2021/stockage/fichierinitial.xlsx')
    ...
    for prof in gretudedossier.user_set.all():
        obj,created=SauvegardeSelection2021.objects.update_or_create(user=prof)
        obj.valeur=dumps([])
        obj.ordre=dumps([])
        #obj.boursier=False
        #obj.pasexpert=False
...
def convertion_xslx():
    ...
    df.to_excel('etudedossier2021/stockage/fichierfinal.xlsx',sheet_name='parcoursup')    
...
def patch_old2():
    ...
    df_new=pd.read_excel('etudedossier2022/stockage/fichierinitial.xlsx')
    engine = create_engine('sqlite:///etudedossier2022/stockage/versionxls.db', echo=False)
    with engine.connect() as conn:
...
def extraction_donnees(request):
    ...
    try:
        ...
        try:
            ...
            AnciensEleves(annee=2021,classe=classe,num_dossier=num_dossier,rne=rne,prenom=prenom,nom=nom,note_initiale=note_initiale,note_finale=note_finale,
