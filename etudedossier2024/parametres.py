...
engine=create_engine('sqlite:///etudedossier2024/stockage/versionxls.db',echo=False)
fichier_lycee=pd.read_csv('etudedossier2024/aa_doc annexes/lycees.csv',sep=";")

def p(s): # protection des noms de colonnes en SQL
    return '`'+s+'`'

lesfichiersPDF=listdir("etudedossier2024/fiches")
associationColonnes={
...
def prepare_selection(context,request):
    def recupere_selection(utilise_sauvegarde=True):
        ...
        if utilise_sauvegarde:
            try:
                obj=SauvegardeSelection2024.objects.get(user=request.user)
                if obj.pasexpert:
        ...
        if "choixAction" in request.POST:
            if request.POST["choixAction"]=="sauvegarde":
                try:
                    ...
                    obj=SauvegardeSelection2024(user=request.user)
...
def liste_anciens(n):
    liste=AnciensEleves.objects.filter(annee=2024,classe="MPSI"+str(n)).order_by("nom")
    l=[]
    for line in liste:
...
def lire_un_dossier(request,context):
    ...
    with engine.connect() as conn :
        ...
        dossier["commentaire_ancien"]=trouve_com_ancien(2024,dossier["numeroDossier"])
...
def convertion_excel():
    global engine
    try:
        remove('etudedossier2024/stockage/versionxls.db')
    except:
        pass
    df=pd.read_excel('etudedossier2024/stockage/fichierinitial.xlsx')
    ...
    for prof in gretudedossier.user_set.all():
        obj,created=SauvegardeSelection2024.objects.update_or_create(user=prof)
        obj.valeur=dumps([])
        obj.ordre=dumps([])
        #obj.boursier=False
        #obj.pasexpert=False
...
def convertion_xslx():
    ...
    df.to_excel('etudedossier2024/stockage/fichierfinal.xlsx',sheet_name='parcoursup')    
...
def patch_old3():
    ...
    df_new=pd.read_excel('etudedossier2024/stockage/fichierinitial.xlsx')
    engine = create_engine('sqlite:///etudedossier2024/stockage/versionxls.db', echo=False)
    with engine.connect() as conn:
...
def extraction_donnees(request):
    ...
    try:
        ...
        f = open("private_files/transfert_fiche/"+login+"_2024json", "w")
        dump(donnees_a_extraire,f)
        f.close()
        for x in lesfichiersPDF:
            if numdossier in x:
                shutil.copy("etudedossier2024/fiches/"+x,"private_files/transfert_fiche/"+login+"_2024.pdf")
