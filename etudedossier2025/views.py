...
from .forms import GestionDossier2025Form
...
def selection(request,err=False):
    ...
    if err:
        context['erreur']="oui"
    return render(request,'etudedossier2025/selection.html',context)

@auth([groupe_etudedossier])
def affichage(request):
    context={}
    lire_tous_les_dossiers(request,context)
    return render(request,'etudedossier2025/affichage.html',context)
...
def gestion(request):
    context={}
    form=GestionDossier2025Form()
    if request.method=='POST' and (request.user.username in ["nizon","rezzouk"]):
        if request.POST["valeur"]=='upload_initial':
            old=GestionDossier2025.objects.all()
            for x in old:
                x.fichier_initial.delete()
                x.delete()
            try:
                remove('etudedossier2025/stockage/fichierinitial.xlsx')
            except:
                pass
            form=GestionDossier2025Form(request.POST,request.FILES) 
            if form.is_valid():
                form.save()
    ...
    context["form"]=form
    return render(request,'etudedossier2025/gestion.html',context)

@auth([groupe_etudedossier])
def download(request,nom):
    try:
        if nom=='fichierfinal.xlsx':
            document=open('etudedossier2025/stockage/fichierfinal.xlsx','rb')
            ...
            document=open('etudedossier2025/fiches/'+nom,'rb')
            response = HttpResponse(FileWrapper(document),content_type='application/octet-stream')
