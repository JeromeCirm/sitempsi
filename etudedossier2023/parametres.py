...
lesfichiersPDF=listdir("etudedossier2023/fiches")
associationColonnes={
...
def prepare_selection(context,request):
    def recupere_selection(utilise_sauvegarde=True):
        ...
        if utilise_sauvegarde:
            try:
                obj=SauvegardeSelection2023.objects.get(user=request.user)
                if obj.pasexpert:
        ...
        if "choixAction" in request.POST:
            if request.POST["choixAction"]=="sauvegarde":
                try:
                    ...
                    obj=SauvegardeSelection2023(user=request.user)
...
def liste_anciens(n):
    liste=AnciensEleves.objects.filter(annee=2023,classe="MPSI"+str(n)).order_by("nom")
    l=[]
    for line in liste:
...
def lire_un_dossier(request,context):
    ...
    with engine.connect() as conn :
        ...
        dossier["commentaire_ancien"]=trouve_com_ancien(2023,dossier["numeroDossier"])
...
def convertion_excel():
    global engine
    try:
        remove('etudedossier2023/stockage/versionxls.db')
    except:
        pass
    ...
    for prof in gretudedossier.user_set.all():
        obj,created=SauvegardeSelection2023.objects.update_or_create(user=prof)
        obj.valeur=dumps([])
        obj.ordre=dumps([])
        #obj.boursier=False
        #obj.pasexpert=False
...
def patch_old3():
    ...
    df_new=pd.read_excel('etudedossier2023/stockage/fichierinitial.xlsx')
    engine = create_engine('sqlite:///etudedossier2023/stockage/versionxls.db', echo=False)
    with engine.connect() as conn:
...
def extraction_donnees(request):
    ...
    try:
        ...
        if request.user.username in ["nizon","bouissou"]:
            ...
            classe=""        
        try:
            obj=AnciensEleves.objects.get(annee=2023,num_dossier=num_dossier)
            obj.commentaire=commentaire
            obj.save()
        except:
            AnciensEleves(annee=2023,classe=classe,num_dossier=num_dossier,rne=rne,prenom=prenom,nom=nom,note_initiale=note_initiale,note_finale=note_finale,
