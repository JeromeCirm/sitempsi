...
from .models import SauvegardeSelection2026
...
fichier_lycee=pd.read_csv('etudedossier2026/aa_doc annexes/lycees.csv',sep=";")

def p(s): # protection des noms de colonnes en SQL
    return '`'+s+'`'

lesfichiersPDF=listdir("etudedossier2026/fiches")
associationColonnes={
...
def prepare_selection(context,request):
    def recupere_selection(utilise_sauvegarde=True):
        ...
        if utilise_sauvegarde:
            try:
                obj=SauvegardeSelection2026.objects.get(user=request.user)
                if obj.pasexpert:
        ...
        if "choixAction" in request.POST:
            if request.POST["choixAction"]=="sauvegarde":
                try:
                    ...
                    obj=SauvegardeSelection2026(user=request.user)
...
def convertion_excel():
    global engine
    try:
        remove('etudedossier2026/stockage/versionxls.db')
    except:
        pass
    df=pd.read_excel('etudedossier2026/stockage/fichierinitial.xlsx')
    ...
    for prof in gretudedossier.user_set.all():
        obj,created=SauvegardeSelection2026.objects.update_or_create(user=prof)
        obj.valeur=dumps([])
        obj.ordre=dumps([])
        #obj.boursier=False
        #obj.pasexpert=False
...
def extraction_donnees(request):
    ...
    try:
        ...
        f = open("private_files/transfert_fiche/"+login+"_2026json", "w")
        dump(donnees_a_extraire,f)
        f.close()
        for x in lesfichiersPDF:
            if numdossier in x:
                shutil.copy("etudedossier2026/fiches/"+x,"private_files/transfert_fiche/"+login+"_2026.pdf")
...
def extraction_donnees(request):
    ...
    try:
        ...
        if request.user.username in ["nizon","bouissou"]:
            ...
            classe=""
        try:
            obj=AnciensEleves.objects.get(annee=2026,num_dossier=num_dossier)
            obj.commentaire=commentaire
            obj.save()
        except:
            AnciensEleves(annee=2026,classe=classe,num_dossier=num_dossier,rne=rne,prenom=prenom,nom=nom,note_initiale=note_initiale,note_finale=note_finale,
