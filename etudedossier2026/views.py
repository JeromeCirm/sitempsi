...
from .parametres import extraction_donnees,extraction_donnees_nouveau,rang_final,test_jerome,rang_fin_phase,convertion_xslx_minimal,sauvegarde_phase_generale,lire_un_dossier,lire_tous_les_dossiers,prepare_selection,convertion_excel,convertion_xslx,maj_dossier,patch
from django.http import HttpResponse
from wsgiref.util import FileWrapper
from django.contrib.auth.models import Group, User
from django.contrib.auth import authenticate, login
from .forms import GestionDossier2026Form
...
def selection(request,err=False):
    ...
    if err:
        context['erreur']="oui"
    return render(request,'etudedossier2026/selection.html',context)

@auth([groupe_etudedossier])
def affichage(request):
    context={}
    lire_tous_les_dossiers(request,context)
    return render(request,'etudedossier2026/affichage.html',context)
...
def traitement(request):
    ...
    if request.method=='POST':
        ...
        elif "extraction_donnees" in request.POST and request.POST["extraction_donnees"]=="eleves":
            context["extraction_msg"]=extraction_donnees_nouveau(request)
...
def gestion(request):
    context={}
    form=GestionDossier2026Form()
    if request.method=='POST' and (request.user.username in ["nizon","rezzouk"]):
        if request.POST["valeur"]=='upload_initial':
            old=GestionDossier2026.objects.all()
            for x in old:
                x.fichier_initial.delete()
                x.delete()
            try:
                remove('etudedossier2026/stockage/fichierinitial.xlsx')
            except:
                pass
            form=GestionDossier2026Form(request.POST,request.FILES) 
            if form.is_valid():
                form.save()
    ...
    context["form"]=form
    return render(request,'etudedossier2026/gestion.html',context)

@auth([groupe_etudedossier])
def download(request,nom):
    try:
        if nom=='fichierfinal.xlsx':
            document=open('etudedossier2026/stockage/fichierfinal.xlsx','rb')
            ...
            document=open('etudedossier2026/fiches/'+nom,'rb')
            response = HttpResponse(FileWrapper(document),content_type='application/octet-stream')
